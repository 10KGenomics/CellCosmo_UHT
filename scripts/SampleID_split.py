...
def parse_index_spec(spec, total_possible_indices=None):
    """
    解析索引规范（兼容三种格式，1-based索引）：
    1. 等差数列：[start,end,step] 如[1,64,8]（1-based，包含起止，步长8）
    2. 范围格式：1-64（1-based，包含起止）
    3. 单个索引：2（1-based）
    
    返回：1-based索引列表（内部直接使用）
    """
    indices = []
    ...
    for part in parts:
        part = part.strip()
        if '-' in part:
            # 范围格式：1-64 → 1-based（包含起止）
            try:
                a, b = map(int, part.split('-'))
                # 重要变更：使用1-based索引，范围检查从1开始
                if a < 1 or b < a:
                    raise ValueError(f"范围 [{a}-{b}] 无效（1-based索引需满足a≥1且b≥a）")
                indices.extend(range(a, b + 1))  # 包含结束位置
            except:
                raise ValueError(f"无效范围格式 '{part}'，应为1-based的start-end（如1-64）")
        else:
            # 单个索引：2 → 1-based
            try:
                idx = int(part)
                # 重要变更：使用1-based索引，范围检查从1开始
                if idx < 1:
                    raise ValueError(f"索引 {idx} 无效（1-based索引需≥1）")
                indices.append(idx)
            except:
                raise ValueError(f"无效单个索引 '{part}'，应为1-based整数（如2）")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--splitCB', nargs='+', required=True,
                        metavar='SPEC',
                        help='1-based索引规范列表，支持：\n'
                             '1. 等差数列：[起始,结束,步长] 如[1,64,8]\n'
                             '2. 范围格式：起始-结束 如1-64\n'
                             '3. 单个索引：2\n'
                             '参数格式：双引号包裹，空格分隔多个规范，如 "[1,64,8] [1,64,8]"')
    ...
    barcode_to_index = {}  # key: 16bp Barcode (第一列), value: 1-based索引 (从bc2-xxx提取)
    ...
    with open(args.inputfileBarcodelist) as f:
        ...
        for line in f:
            ...
            if not match:
                raise ValueError(f"mapping.txt第{line_num}行第二列未找到bc2-xxx格式（如bc2-1），内容：{second_col}")
            
            try:
                idx = int(match.group(1))  # 提取数字部分并转换为整数
                # 重要变更：使用1-based索引，不再减1
                if idx < 1:
                    raise ValueError(f"mapping.txt第{line_num}行索引小于1：{idx}")
            except ValueError:
