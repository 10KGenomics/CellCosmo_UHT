...
for arg in $CB3_NUM; do  # 注意这里不加引号以进行单词分割
    if [[ $arg == *-* ]]; then
        ...
        for ((i = start; i <= end; i++)); do
            echo -e "barcode3_${i},barcode1,barcode2\t01-Fastq_Modify_ParaFly/lib${i}" >> "${Work_Path}/log/${CB3_lib}"
        done
    else
        # 处理单个编号
        if ! [[ "$arg" =~ ^[0-9]+$ ]]; then
        ...
        echo -e "barcode3_${arg},barcode1,barcode2\t01-Fastq_Modify_ParaFly/lib${arg}" >> "${Work_Path}/log/${CB3_lib}"
    fi
done
