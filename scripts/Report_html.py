...
# ---------------------------
# 3.1 解析Merge统计数据（05-Combined/2-Cell3_Merge_Summary.xls，Tab分隔）
# 修正：纯文本解析（保留原有逻辑）
# ---------------------------
...
# ---------------------------
# 3.2 解析Merge Barcode数据（08-Sample_Summary/2-Cell_Summary_withSampleID_AllCB.xls）
# 修正：抽样逻辑（保留原有逻辑）
# ---------------------------
...
def parse_merge_barcode_data(file_path):
    try:
        ...
        return [{"x": i*SAMPLING_INTERVAL+1, "y": val} for i, val in enumerate(sampled_umi_counts)], len(umi_counts)
    except FileNotFoundError:
...
merge_barcode_rank_data, umi_counts_length = parse_merge_barcode_data(merge_barcode_file)
if merge_barcode_rank_data:
    print(f"✓ 解析Merge Barcode数据（原始{umi_counts_length}点，抽样后{len(merge_barcode_rank_data)}点）")

# ---------------------------
# 3.3 解析Sample统计数据（08-Sample_Summary/5-Sample_Summary.xls，Tab分隔）
# 修正：Pandas解析（保留原有逻辑）
# ---------------------------
...
# ---------------------------
# 3.4 解析Sample Barcode数据（08-Sample_Summary/2-Cell_Summary_withSampleID_AllCB.xls）
# 修正：抽样逻辑（保留原有逻辑）
# ---------------------------
...
<body>
    <div class="main-container">
        ...
        <div class="tab-container">
            <button class="tab-button active" data-tab="merge">MergeLibrary</button>
