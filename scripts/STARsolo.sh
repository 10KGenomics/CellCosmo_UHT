...
usage() {
    ...
    echo "  --inputFiles <文件列表>     : 输入文件列表，按R2 R1 R2 R1顺序 (替代inputR1和inputR2)"
    echo "  --soloFeatures <字符串>     : soloFeatures参数 (默认: 'GeneFull GeneFull_Ex50pAS Velocyto')"
    echo "  --soloUMIlen <数字>         : UMI长度 (默认: 8)"
    echo "  --soloBAM_param <字符串>    : STARsoloBAM参数 (默认: '--outSAMtype BAM SortedByCoordinate --outSAMattributes CR UR')"
...
soloBAM_param_default="--outSAMtype BAM SortedByCoordinate --outSAMattributes CR UR"
...
if [[ -z "$STARindex" || -z "$CB3_Num" || -z "$STARsoloThreads" || -z "$TopCell" || -z "$nFastqs" || 
      -z "$SplitcodeNum" || -z "$config" || -z "$inputFiles" ]]; then
    echo "错误: 缺少必需参数!"
    usage
fi
...
: "${soloBAM_param:=$soloBAM_param_default}"
